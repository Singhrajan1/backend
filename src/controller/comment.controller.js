...
const createComment = asyncHandler(async (req, res) => {
  ...
  return res
    .status(201)
    .json(new ApiResponse(201, comment, "Successfully created a Comment"));
});
...
const getVideoComments = asyncHandler(async (req, res) => {
  ...
  const comments = await getComments({ video: videoId }, skip, limit);

  return res
    .status(200)
    .json(new ApiResponse(200, { comments, page, limit }, "Successfully fetched comments"));
});
...
const getPostComments = asyncHandler(async (req, res) => {
  ...
  const comments = await getComments({ post: postId }, skip, limit);
...
const updateComment = asyncHandler(async (req, res) => {
  ...
  if (req.user._id.toString() !== comment.owner.toString()) {
    throw new ApiError(403, "You are not authorized to make changes to the comment");
  }
